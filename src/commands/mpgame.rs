...
pub async fn worduel_challenge(
    ...
    if word.len() < config::MIN_WORDSIZE || word.len() > config::MAX_WORDSIZE {
        ctx.say(format!("**Error:** Word length {} not allowed!", word.len())).await?;
        return Ok(());
    }
    ...
    if user.eq(ctx.author()) {
        ctx.say("**Error:** Cannot challenge yourself! Maybe there will be a singleplayer mode later.").await?;
        return Ok(());
    }
    ...
    if let Some(userdata2) = udlock.get(&user.id) {
        match userdata2.player.game {
            ActiveGame::None => {},
            _ => {
                ctx.say("**Error:** This player is already in a game.").await?;
                return Ok(());
            },
        }
    }
    ...
    { // Own data scope
        ...
        mplock.insert(own_id, GameMP::create(own_id, other_id, word.to_lowercase()));
    ...
    ctx.send(|m| {
        m.content(serenity::MessageBuilder::new()
...
pub async fn worduel_accept(
    ...
    if other_id == own_id {
        ctx.say("**Error:** You can't accept your own challenge!").await?;
        return Ok(());
    }
    ...
    if gamedata.get_word_length() != word.len() {
        ctx.say("**Error:** Word length does not match challenge word.").await?;
        return Ok(());
    }
    if !gamedata.respond(word.to_lowercase()) {
        ctx.say("**Error:** Failed to accept. Do you actually have an invitation waiting?").await?;
        return Ok(());
    }
    ctx.send(|m|
        m.content(serenity::MessageBuilder::new()
...
pub async fn worduel_send(
    ...
    let (progress, stateline, content, views, game_id, other_id) = {
        ...
        match progress {
            Over(Some(i)) => content.push("Game over, ")
                .user(enemy_id)
                .push(", the victor is ")
        ...
        (progress, state.build(), content.build(),
            views, game_id, enemy_id)
    };
