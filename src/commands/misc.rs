...
pub async fn roll_dice(
    ...
    if die_size <= 1 {
        ctx.say(format!("**Error:** {} is not a valid size of dice.", die_size)).await?;
        return Ok(());
    }
    let die_count = die_count.unwrap_or(1);
    ctx.say(format!("Rolling {}d{}...", die_count, die_size)).await?;

    let roll: u64 = {
        /*let mut rng = ctx.data()
            .common_rng
            .lock()
            .await;*/
        ...
        (0..die_count)
            .map(|_| uniform.sample(&mut rng) as u64) // is there a better way?
            .sum()
    };
